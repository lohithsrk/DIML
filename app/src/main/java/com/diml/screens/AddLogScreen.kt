...
@Composable
fun AddLogScreen(navController: NavController, logViewModel: LogViewModel?, log: LogEntity) {
    ...
    Column(
        ...
        TopAppBar(backgroundColor = accent, title = {
            Text(
                text = "Add Log", color = Color.White, fontWeight = FontWeight.ExtraBold
            )
                    ...
                    onClick = {
                        if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(description)) {
                            logViewModel?.addLog(
                                LogEntity(
                                    log.id,
                                    title.replaceFirstChar(Char::titlecase),
...
@RequiresApi(Build.VERSION_CODES.O)
fun AddLogPreview() {
    AddLogScreen(rememberNavController(), null, LogEntity(0, "title", "description"))
}